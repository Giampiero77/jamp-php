...
class cobolDs extends iDS
{
	...
	*/
	public function __construct()
	{
		...
		$this->property["fetch"] 	= "row";
		$this->property["selecteditems"] = "*";
		$this->property["dssavetype"] = "row";

		$this->property["rows"] = null;		
		$this->property["cursor"] = 0;		
	}
	...
	public function dsConnect($dsname=null)
{
	...
	*/
	public function login($post)
	{
		...
		$this->dsQueryFilter(null, $itempwd, $pwd);
		$this->dsConnect();
		$this->dsQuerySelect();
		...
		if (($this->property["row"][$itemuser] == $user) && ($this->property["row"][$itempwd] == $pwd))
		{
			...
			$_SESSION["auth"]["cn"] = $this->property["row"]["cn"];
...
//		if ((empty($qry)) && (isset($this->property["dsdefault"])))
		...
		$this->property["cursor"] = 0;
		unset($this->property["rows"]);
		if (!empty($qry)) 
		{	
			$this->dsQuery($qry);
			list($left, $right) = explode("FROM", $qry);
			if ($this->property["limit"]>0) for ($i=$this->property["start"]; $i<$this->property["limit"]; $i++) $this->property["rows"][]=$this->dsGetRows();
			else while ($row=$this->dsGetRows()) $this->property["rows"][]=$row;
			$this->property["qrycount"] = "SELECT count(".$this->property["dskey"].") AS NUMROWS FROM $right";
		}
	}
	...
	*/
	public function dsGetRows()
	{
		$bool = odbc_fetch_row($this->property["result"]); 
		if ($bool==true) {
			for ($i=1;$i <= odbc_num_fields($this->property["result"]); $i++)
			{
				$field_name = odbc_field_name($this->property["result"], $i);
				$this->property["row"][$field_name] = utf8_encode(odbc_result($this->property["result"], $field_name));
			}
	...
	*/
	...
	{
		if ($row < 0) $row = $this->property["cursor"];
		$this->property["cursor"]++;
		if (isset($this->property["rows"][$row])) return $this->property["row"] = $this->property["rows"][$row];
		return false;
	}
	...
	*/
	public function dsCountRow()
	{
		...
		return (isset($row['NUMROWS'])) ? $row['NUMROWS'] : 0;
	}
