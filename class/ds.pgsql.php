<?php
/**
* Class management ds PostgreSQL
...
class pgsqlDs extends iDS
{
	...
	*/
	public function dsConnect($dsname=null)
	{
		...
		{
			function_exists('pg_connect') or ClsError::showError("DS00", "PGSQL");
			if (empty($dsname)) $dsname = $this->property["dsdefault"];
			if (!empty($dsname)) 
			{
				$conn_string  = "host='".$this->property["dshost"]."'";
				$conn_string .= " port=".(int)$this->property["dsport"];
				$conn_string .= " sslmode='".$this->property["sslmode"]."'";
				if ($this->property["dsuser"]!="none") $conn_string .= " user='".$this->property["dsuser"]."'";
				if ($this->property["dspwd"]!="none") $conn_string .= " password='".$this->property["dspwd"]."'";
				$conn_string .= " dbname=".$dsname;
				$this->property["conn"] = pg_connect($conn_string) or die(pg_last_error($this->property["conn"]));
				pg_query($this->property["conn"], "set client_encoding=\"UTF8\"");
	...
	*/
	...
	{
		$result=pg_exec($this->property["conn"], "SELECT last_value FROM ${tablename}_${fieldname}_seq");
		if ($result) {
	...
	*/
	public function login($post, $chkpwd = true)
	{
		...
		$this->property["row"] = pg_fetch_array($this->property["result"]);
	...
	*/
	public function changepasswd($post)
	{
		...
		return pg_affected_rows($this->property["result"]);
	}
	...
	*/
	public function dsQuerySelect($qry = null)
	{
		...
		if ((empty($qry)) && (isset($this->property["dsdefault"])))
		{
			...
			if ($this->property["limit"] > 0)	$limit = " LIMIT ".$this->property["limit"]." OFFSET ".$this->property["start"];
			else $limit = ""; 
		...
		{ 
			if (!empty($qry))
			{
				...
				{
					$qry .= " LIMIT ".$this->property["limit"]." OFFSET ".$this->property["start"];
				}
			}
		}
	...
	*/
	public function dsQueryInsertUpdateJoin()
	{
		...
		if ($insert) 
		{
			$this->property["inslast"] = $this->pg_insert_id($table, $this->property["dskey"]);
			if (($this->property["inslast"] == 0) && ($this->property["dssavetype"] == "row")) ClsError::showError("DS007");
		}
		else $this->property["inslast"] = 0;
		return pd_affected_rows($this->property["result"]);
	}
	...
	*/
	public function dsQuery($qry, $errorsql = false)
	{
		...
		$this->property["result"] = pg_exec($this->property["conn"], $qry) or $this->ErrorDS003($errorsql);
	...
	*/
	public function dsGetRow($row = -1)
	{
		...
		if ($row >= 0) pg_result_seek($this->property["result"], $row) or ClsError::showError("DS004");
		switch($this->property["fetch"])
		{
			case "object":
				return $this->property["row"] = pg_fetch_object($this->property["result"]);	
			break;
			case "array":
				return $this->property["row"] = pg_fetch_array($this->property["result"]);	
			break;
			case "row":
				return $this->property["row"] = pg_fetch_row($this->property["result"]);	
			break;
			case "assoc":
				return $this->property["row"] = pg_fetch_assoc($this->property["result"]);	
			break;
		}
	}
	...
	*/
	public function dsMoveRow($row)
	{
		...
		if (pg_num_rows($this->property["result"]) == 0) return false;
		pg_result_seek($this->property["result"], $row) or ClsError::showError("DS004");
	}
	...
	*/
	public function dsCountRow()
	{
		...
		{
			if (!empty($this->property["result"]) && !is_bool($this->property["result"])) return pg_num_rows($this->property["result"]);
			return 0;
		} else {
			$res = pg_exec($this->property["conn"], $this->property["qrycount"]) or $this->ErrorDS003($this->property["qrycount"]);
   ...
   * @param string $proc1 name of procedure
	...
	*/
	...
	{
		if (pg_last_error($this->property["conn"]) == 1062)
		{
			$err = pg_last_error($this->property["conn"]);
			$item = explode("'", $err);
			$item = explode("-", $item[1]);
