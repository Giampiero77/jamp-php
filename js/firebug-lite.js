 ...
 * v1.2JAMP
 * 08.08.2008 http://jamp.alyx.it
 */

var firebug = {
	env:{ "cache":{}, "ctmp":[], "cjtmp":[], "catmp":[], "dIndex":"console", "init":false, "ml":false, "objCn":[] },
	init:function(){
		firebug.el = {}; // elements
		firebug.el.content = {};
		with(firebug){
			document.documentElement.childNodes[0].appendChild(
				new pi.element("link").attribute.set("rel","stylesheet").attribute.set("href",firebugcss).environment.getElement()
			);
			...
			el.button.logo = new pi.element("A").attribute.set("title","Firebug Lite").attribute.set("target","_blank").attribute.set("href","http://getfirebug.com/lite.html").update("&nbsp;JAMP Firebug 1.2").attribute.addClass("Button Logo").insert(el.button.container);
			...
			el.nav.jamp    = new pi.element("A").attribute.addClass("Tab").event.addListener("click",d.navigate.curry(window,"jamp")).update("JAMP").insert(el.nav.container);
			el.nav.ajax    = new pi.element("A").attribute.addClass("Tab").event.addListener("click",d.navigate.curry(window,"ajax")).update("AJAX").insert(el.nav.container);
			el.nav.html    = new pi.element("A").attribute.addClass("Tab").event.addListener("click",d.navigate.curry(window,"html")).update("HTML").insert(el.nav.container);
			el.nav.css     = new pi.element("A").attribute.addClass("Tab").event.addListener("click",d.navigate.curry(window,"css")).update("CSS").insert(el.nav.container);
			el.nav.scripts = new pi.element("A").attribute.addClass("Tab").event.addListener("click",d.navigate.curry(window,"scripts")).update("Script").insert(el.nav.container);
			el.nav.dom     = new pi.element("A").attribute.addClass("Tab").event.addListener("click",d.navigate.curry(window,"dom")).update("DOM").insert(el.nav.container);
			el.nav.xhr     = new pi.element("A").attribute.addClass("Tab").event.addListener("click",d.navigate.curry(window,"xhr")).update("XHR").insert(el.nav.container);
			 ...
			 */
			el.left.jamp = {};
			el.left.jamp.container = new pi.element("DIV").attribute.addClass("Console").insert(el.left.container);
			el.left.jamp.mlButton = new pi.element("A").attribute.addClass("MLButton").event.addListener("click",d.console.toggleML).insert(el.left.jamp.container);

			el.left.jamp.monitor = new pi.element("DIV").insert(
				new pi.element("DIV").attribute.addClass("Monitor").insert(el.left.jamp.container)
			);
			
			el.right.jamp = {};
			el.right.jamp.container = new pi.element("DIV").attribute.addClass("Console Container").insert(el.right.container);

			el.right.jamp.mlButton = new pi.element("A").attribute.addClass("MLButton CloseML").event.addListener("click",d.console.toggleML).insert(el.right.jamp.container);

			el.button.jamp = {};
			el.button.jamp.container = new pi.element("DIV").attribute.addClass("ButtonSet JAMP").insert(el.button.container);
			el.button.jamp.clear = new pi.element("A").attribute.addClass("Button").event.addListener("click",d.clean.curry(window,el.left.jamp.monitor)).update("Clear").insert(el.button.jamp.container);		
    		el.button["jamp"].container.environment.addStyle({ "display":"none" });
    		el.left["jamp"].container.environment.addStyle({ "display":"none" });
    		el.right["jamp"].container.environment.addStyle({ "display":"none" });
			
			/*
			 * ajax
			 */
			el.left.ajax = {};
			el.left.ajax.container = new pi.element("DIV").attribute.addClass("Console").insert(el.left.container);
   			el.left.ajax.mlButton = new pi.element("A").attribute.addClass("MLButton CloseML").event.addListener("click",d.console.toggleML).insert(el.left.ajax.container);
			el.left.ajax.monitor = new pi.element("DIV").insert(
				new pi.element("DIV").attribute.addClass("Monitor").insert(el.left.ajax.container)
			);
			
			el.right.ajax = {};
			el.right.ajax.container = new pi.element("DIV").attribute.addClass("Console Container").insert(el.right.container);
			el.right.ajax.mlButton = new pi.element("A").attribute.addClass("MLButton CloseML").event.addListener("click",d.console.toggleML).insert(el.right.ajax.container);

			el.button.ajax = {};
			el.button.ajax.container = new pi.element("DIV").attribute.addClass("ButtonSet AJAX").insert(el.button.container);
			el.button.ajax.clear = new pi.element("A").attribute.addClass("Button").event.addListener("click",d.clean.curry(window,el.left.ajax.monitor)).update("Clear").insert(el.button.ajax.container);
    		el.button["ajax"].container.environment.addStyle({ "display":"none" });
    		el.left["ajax"].container.environment.addStyle({ "display":"none" });
    		el.right["ajax"].container.environment.addStyle({ "display":"none" });
			...
			/*for(var i=0; i<env.cjtmp.length; i++)
			{
				d.jamp.log.apply(window,env.cjtmp[i]);
			}
			
			for(var i=0; i<env.catmp.length; i++)
			{
				d.ajax.log.apply(window,env.catmp[i]);
			}*/
		}	
	},
	win:{
	...
	d: {
		...
		jamp:{
			addLine:function(){
				with (firebug) {
					return new pi.element("DIV").attribute.addClass("Row").insert(el.left.jamp.monitor);
				}
			},
			historyIndex:0,
			history:[],
			...
			scroll:function(){
				with(firebug){
					el.left.jamp.monitor.environment.getElement().parentNode.scrollTop = Math.abs(el.left.jamp.monitor.environment.getSize().offsetHeight-200);
				}
			}
		},
		ajax:{
			addLine:function(){
				with (firebug) {
					return new pi.element("DIV").attribute.addClass("Row").insert(el.left.ajax.monitor);
				}
			},
			historyIndex:0,
			history:[],
			...
			scroll:function(){
				with(firebug){
					el.left.ajax.monitor.environment.getElement().parentNode.scrollTop = Math.abs(el.left.ajax.monitor.environment.getSize().offsetHeight-200);
				}
			}
		},	
		html:{
...
window.consoleJAMP = firebug.d.jamp;
window.consoleAJAX = firebug.d.ajax;
